...
type IAI interface {
	Classify(question string) (string, error)
	Answer(question string, provideAnswers ...string) string
}
...
func (cs *customerService) Classify(question string) (string, error) {
	content := fmt.Sprintf(`%s
Please analyze which of the following intention the above description belongs to, you only need to choose the answer, no explanation is required
- 個人資料
- 公司
- 出差管理辦法
- 員工福利規範
- 員工福利規範
- unknown`, question)
	//resp, err := cs.client.CreateCompletion(
	//	context.Background(),
...
func (cs *customerService) Answer(question string, provideAnswers ...string) string {
