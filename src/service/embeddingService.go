...
func (s *embeddingService) AnswerByChat(question string) (string, error) {
	...
	// 計算向量之間的相似度
	vectorEmbeddingMap := make(map[float64]po.Embedding, len(embeddings))
	similarities := make([]float64, 0, len(embeddings))
	for _, e := range embeddings {
		vectorList := strings.Split(e.Vectors, ";")
		vectors := make([]float64, len(vectorList))
		for i, v := range vectorList {
			vectors[i], _ = strconv.ParseFloat(v, 64)
		}
